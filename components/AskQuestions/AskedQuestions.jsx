...
function AskedQuestions(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get(`https://localhost:44360/api/Question/GetQuestionsByParentId?parentId=${parseInt(userData.Id)}`);
        setData(result.data);
      } catch (error) {
      ...
      {filteredData && filteredData.length > 0 ? (
        ...
        {filteredData.slice(0, visibleQuestions).map((item, index) => (
          <div key={index}  className='Container'>
            <div style={{width:"80vw", backgroundColor: '#dce3f7'}} className='Question-box'>
              <div className='Q'>
